...
const renderSVG = (type) => {
  switch (type) {
    ...
    case "ym":
      return (
        <svg xmlns="http://www.w3.org/2000/svg" width="110" viewBox="0 0 70 70">
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger className="w-[50px] m-auto mt-1">
        <span className="block w-full h-1 mb-2 rounded-sm bg-white"></span>
        <span className="block w-full h-1 mb-2 rounded-sm bg-white"></span>
        <span className="block w-full h-1 rounded-sm bg-white"></span>
      </SheetTrigger>
                ...
                <Sheet>
                  ...
                  >
                    ...
                    <ul className="max-w-[450px] grid grid-cols-1 gap-y-8">
                      {servicesMenu.map(({ title, svg, desc, link }, i) => (
                        <li key={i}>
                          <Link
                            href={link}
